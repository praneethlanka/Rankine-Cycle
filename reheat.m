...
[a,~]=elementfinder(rhsuph,1); %variable a holds the positions of all superheated states
[b,~]=elementfinder(rhsuph,0); %variable b holds the positions of all saturated states
...
rhentsup=XSteam('s_pT',rhp(rhsuph==1),rht(rhsuph==1));
...
rhent(a(:,1),1)=rhentsup(:,1);
rhenth(b(:,1),1)=XSteam('hV_p',rhp(rhsuph==0));
rhent(b(:,1),1)=XSteam('sV_p',rhp(rhsuph==0));
